...
# Safe requests import to prevent startup crashes on Streamlit Cloud
try:
    import requests
    HAS_REQUESTS = True
except ImportError:
...
# 2. Dynamic Path Resolution (Works locally and on Streamlit Cloud)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
MAIN_PY_PATH = os.path.join(BASE_DIR, "main.py")
SUBMISSION_JSON_PATH = os.path.join(BASE_DIR, "submission.json")

# 3. Custom Badge & Layout Styling
st.markdown("""
<style>
    .stMetric {
        background: #161b22;
...
# 4. Session State Initialization for HITL Overrides
...
def load_submission_data(file_path: str = SUBMISSION_JSON_PATH) -> Dict[str, Any]:
    """Loads and caches submission JSON results safely."""
...
def check_server_health(url: str) -> bool:
    """Safely checks if the FastAPI/Docker evaluation server is reachable."""
    if not HAS_REQUESTS:
        return False
    try:
...
with st.sidebar:
    ...
    if st.button("▶️ Run Audit Pipeline", type="primary", use_container_width=True):
        if not os.path.exists(MAIN_PY_PATH):
            st.error(f"Cannot find `main.py` at expected path: `{MAIN_PY_PATH}`. Ensure `main.py` is in the same directory as `app.py`.")
        else:
...
# Build DataFrame with Supervisor Overrides Applied
rows = []
